            ...
            for i in range(len(layer.W)):
                for j in range(len(layer.W[i])):
                    ...
                    if (dw_sign[i][j] > 0): #if the error kept the same sign
                        layer.rprop_deltas[i][j] = min(layer.rprop_deltas[i][j] * self.eta_pos, self.eta_max) #update the rprop_delta, increasing it by eta_pos until we hit eta_max
                        ...
                        layer.rprop_deltas[i][j] = max(layer.rprop_deltas[i][j] * self.eta_neg, self.eta_min) #decrease rprop_delta until we hit eta_min
